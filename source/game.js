...
scene.add(new THREE.HemisphereLight(0xb1d2df,0x575143,2.05));
...
const boxGeo=new THREE.BoxGeometry(1,1,1), cylGeo=new THREE.CylinderGeometry(1,1,1,12), sphereGeo=new THREE.SphereGeometry(1,16,12);
...
function box(m,x,y,z,w,h,d,p=interior){return mesh(boxGeo,m,x,y,z,w,h,d,p);}
...
function ring(x,y,z,r,t=.08,m='steel',p=interior){return mesh(new THREE.TorusGeometry(r,t,8,32),m,x,y,z,1,1,1,p);}
...
function screen(title,kind=0){const m=canvasMat(512,340,(c,W,H)=>{c.fillStyle='#071820';c.fillRect(0,0,W,H);c.strokeStyle='#32535a';c.lineWidth=1;for(let x=25;x<W;x+=32){c.beginPath();c.moveTo(x,60);c.lineTo(x,H-25);c.stroke();}for(let y=60;y<H;y+=28){c.beginPath();c.moveTo(20,y);c.lineTo(W-20,y);c.stroke();}c.fillStyle='#b8cfbf';c.font='bold 20px monospace';c.fillText(title,24,36);c.fillStyle='#e2ba74';c.fillRect(24,47,100,3);if(kind===1){c.strokeStyle='#a9d3b7';for(let r=25;r<110;r+=25){c.beginPath();c.arc(250,195,r,0,Math.PI*2);c.stroke();}c.fillStyle='#f4b363';c.beginPath();c.arc(309,135,7,0,7);c.fill();c.font='15px monospace';c.fillText('TAU CETI   /   LOCKED',34,316);}else if(kind===2){c.fillStyle='#efc377';c.font='bold 66px monospace';c.fillText('1.00 g',85,162);c.font='16px monospace';c.fillStyle='#b0d4c4';c.fillText('TETHER SYSTEM / READY',55,209);c.fillText('DEPLOYMENT AUTHORIZED',55,244);}else{c.strokeStyle='#a4d6b6';c.lineWidth=3;c.beginPath();for(let x=24;x<490;x++){const y=190+Math.sin(x*.055)*35+Math.sin(x*.15)*9;c.lineTo(x,y);}c.stroke();c.font='17px monospace';c.fillStyle='#c6d9bc';c.fillText('SAMPLE A   96.4%   NOMINAL',24,297);}});screenMaterials.push(m);return m;}
...
function monitor(x,y,z,w=1.6,index=0,rot=0){const p=new THREE.Group();p.position.set(x,y,z);p.rotation.y=rot;interior.add(p);box('black',0,0,0,w+.15,w*.68+.15,.15,p);mesh(new THREE.PlaneGeometry(w,w*.68),displays[index],0,0,.081,1,1,1,p);for(let i=0;i<5;i++)box(i===0?'cyan':'steel',-.4+i*.16,-w*.38,.09,.06,.035,.015,p);return p;}
...
function shell(r){const {x1,x2,z1,z2,y}=r,w=x2-x1,d=z2-z1,cx=(x1+x2)/2,cz=(z1+z2)/2,H=4.5;
 ...
 for(const [a,b,c,dz]of patches){box('dark',(a+b)/2,y-.16,(c+dz)/2,b-a,.25,dz-c);for(let x=a;x<b-.01;x+=2)for(let z=c;z<dz-.01;z+=2){const tw=Math.min(2,b-x),td=Math.min(2,dz-z);const o=mesh(new THREE.PlaneGeometry(tw-.025,td-.025),floorMat,x+tw/2,y+.015,z+td/2);o.rotation.x=-Math.PI/2;}}
 if(r.id==='lab'){
...
function bench(x,z,w=3,d=1.2,rot=0){const p=new THREE.Group();p.position.set(x,0,z);p.rotation.y=rot;interior.add(p);box('ivory',0,.95,0,w,.14,d,p);box('dark',0,.42,0,w-.18,.92,d-.12,p);for(let i=0;i<3;i++){box('panel',-w*.33+i*w*.33,.52,d/2+.005,w*.29,.59,.03,p);box('steel',-w*.33+i*w*.33,.7,d/2+.035,.45,.035,.04,p);}collider(x,z,Math.abs(Math.cos(rot))*w+Math.abs(Math.sin(rot))*d,Math.abs(Math.sin(rot))*w+Math.abs(Math.cos(rot))*d);return p;}
...
const viewportMat=canvasMat(512,512,(c,W,H)=>{c.fillStyle='#020b16';c.fillRect(0,0,W,H);let seed=42;const rand=()=>((seed=(seed*1664525+1013904223)>>>0)/4294967296);for(let i=0;i<160;i++){c.fillStyle=i%4?'#c8ddd7':'#dec293';const s=rand()*1.5;c.fillRect(rand()*W,rand()*H,s,s);}const g=c.createRadialGradient(310,220,10,310,220,110);g.addColorStop(0,'#bab484');g.addColorStop(.65,'#777d63');g.addColorStop(.9,'#315052');g.addColorStop(1,'#030d17');c.fillStyle=g;c.beginPath();c.arc(310,220,110,0,7);c.fill();c.strokeStyle='#b3c5aa';c.lineWidth=2;c.beginPath();c.arc(310,220,111,3.5,5.3);c.stroke();});
const windowViews=[];
...
for(const z of [-4,4]){box('ivory',16,.45,z,9,.65,1.1);box('soil',16,.8,z,8.8,.08,.9);collider(16,z,9,1.1);for(let i=0;i<20;i++){const x=11.9+i*.43;const h=.35+(i%4)*.12;beam([x,.82,z],[x,1+h,z],.02,'green');for(let j=0;j<4;j++){const leaf=ball('green',x+(j%2?-.13:.13),1+j*.13,z+Math.sin(j)*.12,.17);leaf.scale.set(.24,.065,.12);leaf.rotation.z=j%2?.5:-.5;}}box('glow',16,2.1,z,9,.06,.16);beam([11.6,2.1,z],[20.4,2.1,z],.04,'steel');}
...
function batchStatic(root,skip=[]){root.updateMatrixWorld(true);const buckets=new Map(),remove=[];root.traverse(o=>{if(!o.isMesh||skip.some(s=>o===s||isChild(o,s)))return;if(!buckets.has(o.material))buckets.set(o.material,[]);const g=o.geometry.clone().applyMatrix4(o.matrixWorld);buckets.get(o.material).push(g);remove.push(o);});for(const o of remove)o.removeFromParent();for(const [m,gs]of buckets){const merged=mergeGeometries(gs,false);if(merged)root.add(new THREE.Mesh(merged,m));gs.forEach(g=>g.dispose());}}
...
function menu(){if(modalOpen)return closeModal();openModal(`<div class="eyebrow">HAIL MARY / FLIGHT MANUAL</div><h2>Take your time.</h2><p>Explore the ship, inspect instruments, and deploy the cable centrifuge. Progress is saved on this device.</p><div class="row"><span>Walk / run</span><span><kbd>W A S D</kbd> <kbd>SHIFT</kbd></span></div><div class="row"><span>Look</span><span>Mouse / drag / arrow keys</span></div><div class="row"><span>Interact / enter ladder</span><kbd>E</kbd></div><div class="row"><span>Climb ladder</span><kbd>W / S</kbd></div><div class="row"><span>Exterior / centrifuge / map</span><span><kbd>C</kbd><kbd>G</kbd><kbd>M</kbd></span></div><div class="row"><span>Jump</span><kbd>SPACE</kbd></div><div class="row"><span>Mouse sensitivity</span><input id="sensitivity" aria-label="Mouse sensitivity" type="range" min="0.3" max="2" step=".1" value="${state.sensitivity}"></div><div class="row"><span>Ship ambience</span><input id="volume" aria-label="Ship ambience volume" type="range" min="0" max="1" step=".05" value="${state.volume}"></div><button class="primary" id="resume">${started?'RESUME EXPLORATION':'ENTER THE SHIP'} <span>↗</span></button><p style="font-size:10px">An original fan-made interpretation of the movie ship. Geometry and sounds are procedural. The room plan and transition timing are adapted for play.</p>`);$('resume').onclick=()=>{closeModal();if(!started)start();};$('sensitivity').oninput=e=>{state.sensitivity=Number(e.target.value);save();};$('volume').oninput=e=>{state.volume=Number(e.target.value);if(ambient)ambient.gain.gain.value=state.volume*.045;save();};}
...
function render(now){requestAnimationFrame(render);const dt=Math.min((now-lastTime)/1000,.04);lastTime=now;time+=dt;
 ...
 sampleGroup.rotation.y=time*.2;updateInteraction();renderer.render(scene,camera);
}
...
document.addEventListener('pointerlockchange',()=>{keys.clear();if(!document.pointerLockElement&&started&&!externalMode&&!modalOpen&&!paused)menu();});
...
window.hailMary={getState:()=>({started,paused,externalMode,position:player.pos.toArray(),yaw:player.yaw,pitch:player.pitch,ground:player.ground,climbing:!!player.climb,room:currentRoom,visited:[...state.visited],done:[...state.done],deploy,rpm,gravity:gravity(),interaction:nearest?.id,drawCalls:renderer.info.render.calls,triangles:renderer.info.render.triangles}),layout:rooms.map(({id,x1,x2,z1,z2,y})=>({id,x1,x2,z1,z2,y})),canStand:(x,z,y)=>canStand(x,z,y)};
